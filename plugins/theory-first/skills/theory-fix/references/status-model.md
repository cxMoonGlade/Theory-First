...
The supported runtime unit is the complete seven-skill suite. Before scientific
work begins, each dependency-bearing skill must resolve all of its named child
skills through the host's native skill discovery mechanism:

- `theory-first` requires `map-research-landscape`, `close-literature`,
  `deep-read-paper`, and `preregister-claim`;
- `theory-fix` requires `close-literature`, `deep-read-paper`, and
  `stress-test-claim`; and
- `close-literature` requires `deep-read-paper`.

If any required child is unavailable, return `SUITE_INCOMPLETE` with the exact
missing names and instructions to install the full suite. Stop before producing
a scientific gate or evidence status. `SUITE_INCOMPLETE` is an installation
diagnostic, not evidence that the claim is blocked, repaired, accepted, or
false. Never inline, summarize, or improvise a missing child workflow.
