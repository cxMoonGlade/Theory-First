...
协作锁只会串行化遵守该锁的 Theory First 安装器。安装进行时，不要让其他 skill manager
或外部进程修改同一组七个 skill 名称或 `.theory-first-*` 事务目录。安装器会保留并报告
已经检测到的 identity 或 content 变化，但不声称能对不遵守该锁的 writer 提供原子保护。
